...
|| @version	1.0
|| @description
|| | Runs some neat effects on a NeoPixel board.
|| #
||
|| @changelog
...
//
// AUTOMATA_LEVEL sets the brightness level of a living automata cell
//
#define NUM_PHASES 24
#define MODULATION_STEPS 47
#define AUTOMATA_LEVEL 64
  ...
  for (uint8_t step = 1; step <= steps; step +=10) {
    for (uint8_t i = 0; i < strip.numPixels(); i++) {
      for (uint8_t c = 0; c < NUM_CHANNELS; c++) {
        ...
        // Scale values greater than MAX_BRIGHTNESS
        if (AUTOMATA_LEVEL > MAX_BRIGHTNESS) {
          step_values[c] /= (AUTOMATA_LEVEL / MAX_BRIGHTNESS);
        }

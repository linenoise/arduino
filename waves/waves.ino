...
void loop() {
  ...
  for (int channel = 0; channel < 3; channel++) {
    ...
    if (block_start[channel] < 0) {
      block_start[channel] = NUM_LEDS - 1;
    }
    ...
    for (int pos = block_start[channel]; pos < block_end[channel]; pos++) {
      int offset = block_end[channel] - pos;
      int pos_wrapped = pos;
      if (pos_wrapped > NUM_LEDS - 1) {
        pos_wrapped = pos_wrapped - NUM_LEDS;
      }
      int value = 100 * sin(double(offset) / block_length[channel] * 3.14159);
      pixel_fader.set_pixel(pos_wrapped, value, channel);
    }
  }

...
|| | In this demonstration, each pixel on the strip gently twinkles.
|| #
||
|| @changelog
...
void loop() {
  for (int i = 0; i < NUM_LEDS; i++) {
    int *pixel = pixel_fader.get_pixel(i);
    for (int c = 0; c < 3; c++) {
      pixel_fader.set_pixel(i, ((pixel[c] * 1.5 + random(0,50)) / 2.5), c);
    }
  }
  pixel_fader.push(100);
}
